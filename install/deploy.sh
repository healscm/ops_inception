...
# 02
Data="02) Install Ningx, Please wait..."
echo -n $Data
...
# 05
Data="05) Git Clone ops, Please wait..."
echo -n $Data
cd /opt && rm -fr ops_back &> /dev/null && mv ops ops_back &> /dev/null
git clonegit@git.gungunqian.cn:qiangungun/ops_inception.git &>/dev/null || failure "$Data"
cd /opt/ops/src &> /dev/null
pip3 install -r requirements.txt &>/dev/null && success "$Data" || failure "$Data" 

# 06
Data="06) Copy ops File, Please wait..."
echo -n $Data
ps aux | grep runserver | grep -v grep | awk '{print $2}' | xargs kill -9 &> /dev/null
rm -fr /usr/share/nginx/html/* &> /dev/null
yes | cp -fnr /opt/ops/install/connections.py /usr/local/lib/python3.6/site-packages/pymysql/ &> /dev/null 
yes | cp -fnr /opt/ops/install/cursors.py /usr/local/lib/python3.6/site-packages/pymysql/ &> /dev/null
yes | cp -fnr /opt/ops/webpage/dist/* /usr/share/nginx/html/ &>/dev/null && success "$Data" || failure "$Data"

# 07
Data="07) Start Ningx, Please wait..."
echo -n $Data
if [ $VERSION = 2 ];then
...
if [ $? = 0 ];then
  mysql -uroot -p"$MYSQLPASSWORD" -e "create database if not exists ops charset utf8;" &> /dev/null
else
  mysql -uroot -p"$MYSQLPASSWORD" -e "create database if not exists ops charset utf8;" &> /dev/null
fi
...
cd /opt/ops/src && sed -i "s/backuppassword =.*/backuppassword = $MYSQLPASSWORD/" deploy.conf &> /dev/null
cd /opt/ops/src && sed -i "s/ipaddress = .*/ipaddress = ${ADDRESS}:8000/" deploy.conf &> /dev/null
cd /opt/ops/src && sed -i "s/password =.*/password = $MYSQLPASSWORD/" deploy.conf &> /dev/null 

# 11
Data="11) Migrate ops Tables, Please wait..."
echo -n $Data
cd /opt/ops/src
python3 manage.py makemigrations &> /dev/null && python3 manage.py migrate &> /dev/null && success "$Data" || failure "$Data"

# 12
read -p "12) Set ops admin User Passwrod: " PASSWORD
if [ -z $PASSWORD ];then
   echo -e "Input cannot empty, please enter again"
   read -p "Set ops admin User Passwrod: " PASSWORD
fi
...
echo -n $Data
cd /opt/ops/install/ && tar xvf inception.tar &> /dev/null
ps aux | grep Inception | grep -v grep | awk '{print $2}' | xargs kill -9 &> /dev/null
/opt/ops/install/inception/bin/Inception --defaults-file=/opt/ops/install/inception/bin/inc.cnf &> /dev/null & 
if [ $? = 0 ];then
  success "$Data" 
else
  failure "$Data"
...
Inception conf |   /opt/ops/install/inception/bin/inc.cnf |
ops conf  |   /opt/ops/src/deploy.conf               |
ops log   |   /opt/ops/src/log/*                     |
----------------------------------------------------------------
END
}
